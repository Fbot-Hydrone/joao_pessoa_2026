// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from hydrone_msgs:srv/SetPhase.idl
// generated code does not contain a copyright notice
#include "hydrone_msgs/srv/detail/set_phase__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

bool
hydrone_msgs__srv__SetPhase_Request__init(hydrone_msgs__srv__SetPhase_Request * msg)
{
  if (!msg) {
    return false;
  }
  // phase
  // open_hardware
  // use_two_drones
  return true;
}

void
hydrone_msgs__srv__SetPhase_Request__fini(hydrone_msgs__srv__SetPhase_Request * msg)
{
  if (!msg) {
    return;
  }
  // phase
  // open_hardware
  // use_two_drones
}

bool
hydrone_msgs__srv__SetPhase_Request__are_equal(const hydrone_msgs__srv__SetPhase_Request * lhs, const hydrone_msgs__srv__SetPhase_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // phase
  if (lhs->phase != rhs->phase) {
    return false;
  }
  // open_hardware
  if (lhs->open_hardware != rhs->open_hardware) {
    return false;
  }
  // use_two_drones
  if (lhs->use_two_drones != rhs->use_two_drones) {
    return false;
  }
  return true;
}

bool
hydrone_msgs__srv__SetPhase_Request__copy(
  const hydrone_msgs__srv__SetPhase_Request * input,
  hydrone_msgs__srv__SetPhase_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // phase
  output->phase = input->phase;
  // open_hardware
  output->open_hardware = input->open_hardware;
  // use_two_drones
  output->use_two_drones = input->use_two_drones;
  return true;
}

hydrone_msgs__srv__SetPhase_Request *
hydrone_msgs__srv__SetPhase_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hydrone_msgs__srv__SetPhase_Request * msg = (hydrone_msgs__srv__SetPhase_Request *)allocator.allocate(sizeof(hydrone_msgs__srv__SetPhase_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(hydrone_msgs__srv__SetPhase_Request));
  bool success = hydrone_msgs__srv__SetPhase_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
hydrone_msgs__srv__SetPhase_Request__destroy(hydrone_msgs__srv__SetPhase_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    hydrone_msgs__srv__SetPhase_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
hydrone_msgs__srv__SetPhase_Request__Sequence__init(hydrone_msgs__srv__SetPhase_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hydrone_msgs__srv__SetPhase_Request * data = NULL;

  if (size) {
    data = (hydrone_msgs__srv__SetPhase_Request *)allocator.zero_allocate(size, sizeof(hydrone_msgs__srv__SetPhase_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = hydrone_msgs__srv__SetPhase_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        hydrone_msgs__srv__SetPhase_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
hydrone_msgs__srv__SetPhase_Request__Sequence__fini(hydrone_msgs__srv__SetPhase_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      hydrone_msgs__srv__SetPhase_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

hydrone_msgs__srv__SetPhase_Request__Sequence *
hydrone_msgs__srv__SetPhase_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hydrone_msgs__srv__SetPhase_Request__Sequence * array = (hydrone_msgs__srv__SetPhase_Request__Sequence *)allocator.allocate(sizeof(hydrone_msgs__srv__SetPhase_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = hydrone_msgs__srv__SetPhase_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
hydrone_msgs__srv__SetPhase_Request__Sequence__destroy(hydrone_msgs__srv__SetPhase_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    hydrone_msgs__srv__SetPhase_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
hydrone_msgs__srv__SetPhase_Request__Sequence__are_equal(const hydrone_msgs__srv__SetPhase_Request__Sequence * lhs, const hydrone_msgs__srv__SetPhase_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!hydrone_msgs__srv__SetPhase_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
hydrone_msgs__srv__SetPhase_Request__Sequence__copy(
  const hydrone_msgs__srv__SetPhase_Request__Sequence * input,
  hydrone_msgs__srv__SetPhase_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(hydrone_msgs__srv__SetPhase_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    hydrone_msgs__srv__SetPhase_Request * data =
      (hydrone_msgs__srv__SetPhase_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!hydrone_msgs__srv__SetPhase_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          hydrone_msgs__srv__SetPhase_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!hydrone_msgs__srv__SetPhase_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `message`
#include "rosidl_runtime_c/string_functions.h"

bool
hydrone_msgs__srv__SetPhase_Response__init(hydrone_msgs__srv__SetPhase_Response * msg)
{
  if (!msg) {
    return false;
  }
  // success
  // message
  if (!rosidl_runtime_c__String__init(&msg->message)) {
    hydrone_msgs__srv__SetPhase_Response__fini(msg);
    return false;
  }
  return true;
}

void
hydrone_msgs__srv__SetPhase_Response__fini(hydrone_msgs__srv__SetPhase_Response * msg)
{
  if (!msg) {
    return;
  }
  // success
  // message
  rosidl_runtime_c__String__fini(&msg->message);
}

bool
hydrone_msgs__srv__SetPhase_Response__are_equal(const hydrone_msgs__srv__SetPhase_Response * lhs, const hydrone_msgs__srv__SetPhase_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  // message
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->message), &(rhs->message)))
  {
    return false;
  }
  return true;
}

bool
hydrone_msgs__srv__SetPhase_Response__copy(
  const hydrone_msgs__srv__SetPhase_Response * input,
  hydrone_msgs__srv__SetPhase_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  // message
  if (!rosidl_runtime_c__String__copy(
      &(input->message), &(output->message)))
  {
    return false;
  }
  return true;
}

hydrone_msgs__srv__SetPhase_Response *
hydrone_msgs__srv__SetPhase_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hydrone_msgs__srv__SetPhase_Response * msg = (hydrone_msgs__srv__SetPhase_Response *)allocator.allocate(sizeof(hydrone_msgs__srv__SetPhase_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(hydrone_msgs__srv__SetPhase_Response));
  bool success = hydrone_msgs__srv__SetPhase_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
hydrone_msgs__srv__SetPhase_Response__destroy(hydrone_msgs__srv__SetPhase_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    hydrone_msgs__srv__SetPhase_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
hydrone_msgs__srv__SetPhase_Response__Sequence__init(hydrone_msgs__srv__SetPhase_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hydrone_msgs__srv__SetPhase_Response * data = NULL;

  if (size) {
    data = (hydrone_msgs__srv__SetPhase_Response *)allocator.zero_allocate(size, sizeof(hydrone_msgs__srv__SetPhase_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = hydrone_msgs__srv__SetPhase_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        hydrone_msgs__srv__SetPhase_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
hydrone_msgs__srv__SetPhase_Response__Sequence__fini(hydrone_msgs__srv__SetPhase_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      hydrone_msgs__srv__SetPhase_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

hydrone_msgs__srv__SetPhase_Response__Sequence *
hydrone_msgs__srv__SetPhase_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hydrone_msgs__srv__SetPhase_Response__Sequence * array = (hydrone_msgs__srv__SetPhase_Response__Sequence *)allocator.allocate(sizeof(hydrone_msgs__srv__SetPhase_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = hydrone_msgs__srv__SetPhase_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
hydrone_msgs__srv__SetPhase_Response__Sequence__destroy(hydrone_msgs__srv__SetPhase_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    hydrone_msgs__srv__SetPhase_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
hydrone_msgs__srv__SetPhase_Response__Sequence__are_equal(const hydrone_msgs__srv__SetPhase_Response__Sequence * lhs, const hydrone_msgs__srv__SetPhase_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!hydrone_msgs__srv__SetPhase_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
hydrone_msgs__srv__SetPhase_Response__Sequence__copy(
  const hydrone_msgs__srv__SetPhase_Response__Sequence * input,
  hydrone_msgs__srv__SetPhase_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(hydrone_msgs__srv__SetPhase_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    hydrone_msgs__srv__SetPhase_Response * data =
      (hydrone_msgs__srv__SetPhase_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!hydrone_msgs__srv__SetPhase_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          hydrone_msgs__srv__SetPhase_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!hydrone_msgs__srv__SetPhase_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
